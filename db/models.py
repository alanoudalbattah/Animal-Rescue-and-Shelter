...
# src: https://docs.sqlalchemy.org/en/14/orm/basic_relationships.html for
# relationships :)

LOCAL_DATABASE_URL = "postgresql://{}:{}@{}/{}".format(
    'postgres', 'postgres', 'localhost:5432', 'animal_shelter')
...
def setup_db(app, database_path=None):
    ...
    if heroku_postgres_path is not None:
        app.config["SQLALCHEMY_DATABASE_URI"] = heroku_postgres_path.replace(
            "://", "ql://", 1)
    # local database for the unit tests :) else default it to none so the
    # local database for devolopment can be used
...
class Adoption_Interview(db.Model):
    ...
    pet_2b_adopted = db.relationship(
        "Pet", backref=db.backref(
            "interview", uselist=False))
    ...
    # src:
    # https://www.codegrepper.com/code-examples/python/convert+datetime+object+into+string+sqlalchemy
