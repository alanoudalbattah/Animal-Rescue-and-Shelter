...
# However, i prioritized documentation and authorization for the sake of
# time :)
...
class AnimalShelter(unittest.TestCase):
    ...
    def create_temp_specie(self):
        return self.client().post('/specie',
                                  data=json.dumps({'specie': 'Cat'}),
    ...
    def test_400_create_specie(self):
        ...
        res = self.client().post('/specie',
                                 data=json.dumps({'specie': 'Fish'}),
                                 headers={'Content-Type': 'application/json'})
        data = json.loads(res.data)
    ...
    def test_200_update_specie_byID(self):
        ...
        res = self.client().patch('/specie/' + str(avalible_id),
    ...
    def test_200_update_breed_byID(self):
        ...
        res = self.client().patch('/breed/' + str(avalible_breed.id),
                                  data=json.dumps({'specie_id': avalible_specie.id}),
                                  headers={'Content-Type': 'application/json'})
        data = json.loads(res.data)
    ...
    def test_400_update_breed_byID(self):
        ...
        res = self.client().patch('/breed/' + str(avalible_id),
                                  data=json.dumps({'specie_id': '8'}),
                                  headers={'Content-Type': 'application/json'})
        # test status code
        self.assertEqual(res.status_code, 400)
