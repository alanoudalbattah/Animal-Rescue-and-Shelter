...
def create_app():
  ...
  '''
    ...
    def create_user():
        ...
        for required in required_data:
            if (required is None):
                abort(400, description='required feild is missing')
    ...
    def view_all_pets():
        ...
        if (len(paginated_pets) == 0):
            # abort if no questions were formatted (no need for a new page)!
            abort(404)
  ...
  '''
    ...
    def display_user_interviews(payload, _user_id):
        return jsonify(
            {
                "interviews": [
                    interview.id for interview in Adoption_Interview.query.filter(
                        User.id == _user_id).all()]}), 200
  ...
  '''
    ...
    def book_interview(payload):
        ...
        if (pet is None or user is None):
            abort(400, description="pet or user are missing from the db")
  ...
  '''
    ...
    def view_upcoming_interviews(payload, _id):
        ...
        if request.method == "PATCH":
            ...
            if ('hour' in body):
                interview.time = time(
                    body.get('hour'), interview.time.minute, 0)
            if ('minute' in body):
                interview.time = time(
                    interview.time.hour, body.get('minute'), 0)
            ...
            return jsonify({"interview details": interview.details()}), 200
  ...
  '''
    ...
    def view_all_interviews(payload):
        return jsonify({'all interviews': [Adoption_Interview.details(
            interview) for interview in Adoption_Interview.query.all()]}), 200
    ...
    def create_cat_information(payload):
        ...
        if 'breed' in body:
            ...
            breed = Breed.query.filter(
                Breed.name.ilike(f'%{search_term}%')).first()
            if not(breed):
                abort(400, description="breed name not avalible, please create one")
  ...
  '''
    ...
    def view_all_adopted_pets(payload):
        return jsonify({'adopted pets': [Pet.details(
            pet) for pet in Pet.query.all() if pet.pet_owner is not None]}), 200
  ...
  '''
    ...
    # other permmisions dose the same maybe i should delete them? 'patch:pet',
    # 'delete:pet'
    @requires_auth('post:pet')
    def pet_cat(payload, _id):
        ...
        if request.method == "PATCH":
            ...
            if ('breed' in body):
                ...
                # search is case insensitive :)
                breed = Breed.query.filter(
                    Breed.name.ilike(f'%{breedName}%')).first()
                if not (breed):
                    abort(400, description='breed name is incorrect')
                # if it exists ...
                pet.breed = breed
            ...
            try:
                ...
                abort(
                    400,
                    description='constraint violation could not be deleted, maybe there is an interview linked :)')
  ...
  '''
    ...
    def create_new_breed(payload):
        ...
        if not('specie' in body):
            abort(400, description='specie is not included in the body')
        specieName = body.get('specie')
        simmilar = Specie.name.ilike(f'%{specieName}%')
        specie = Specie.query.filter(simmilar).first()
        if not('breed' in body):
            abort(400, description='breed is not included in the body')

        breedName = body.get('breed')

        # check if breed already exists in the database
        # search is case insensitive :)
        breed = Breed.query.filter(Breed.name.ilike(f'%{breedName}%')).first()
        if (breed):
            abort(400, description='breed name already exists')
    ...
    def breed(payload, _id):
        ...
        if request.method == "PATCH":
            ...
            try:
                ...
                # unique name constraint violation or charecters extend the
                # limits
                abort(400, description='constraint violation could not be updated')
            ...
            try:
                ...
                abort(
                    400,
                    description='constraint violation could not be deleted, maybe there is a pet linked')

            return jsonify({"breed_id": _id}), 200

        else:
            return jsonify({"breed details": breed.details()}), 200
  ...
  '''
    ...
    def create_new_specie(payload):
        ...
        # search is case insensitive :)
        specie = Specie.query.filter(
            Specie.name.ilike(f'%{specieName}%')).first()
        if (specie):
            abort(400, description='specie name already exists')
  ...
  '''
    ...
    def view_all_specie(payload):
        return jsonify({'all species': [Specie.details(s)
                       for s in Specie.query.all()]}), 200
  ...
  '''
    ...
    def specie(payload, _id):
        ...
        if request.method == "PATCH":
            ...
            return jsonify({"specie details": specie.details()}), 200
